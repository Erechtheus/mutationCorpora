...
def convert_brat_to_document(
    ...
    for brat_span in dl_to_ld(brat_doc["spans"]):
        ...
        if convert_multi_spans:
            if len(locations) > 1:
                ...
                added_fragments_filtered = [
                    frag for frag in added_fragments if frag != GLUE_TEXT
                ]
    ...
    for sent_idx, sentence in enumerate(brat_doc["sentences"]):
        ...
        doc.add_annotation(
            name="sentences",
            annotation=LabeledSpan(start=start, end=end, label=str(sent_idx)),
        )
...
def _convert_brats(brat_docs: Iterable[Dict[str, Any]], **kwargs) -> List[Document]:
    return [
        convert_brat_to_document(brat_doc=brat_doc, **kwargs) for brat_doc in brat_docs
    ]
...
def load_mutations(
    ...
    **kwargs,
    ...
    logger.info(
        f"loaded Mutations documents: " + str({k: len(v) for k, v in data.items()})
    )

    # there is no train data if the dataset is only prepare for prediction
    if "train" in data:
        # get 20% of the train set to serve as dev set
        train_dev = data["train"].train_test_split(test_size=0.1, seed=42)
...
def split_span_annotation(
    text: str, slice: Tuple[int, int], glue: str
) -> List[Tuple[int, int]]:
    """
...
def serialize_binary_relation(
    ...
    serialized_annotation = f"{annotation.label} {head_argument_name}:T{_head_id} {tail_argument_name}:T{_tail_id}"
    ...
    **kwargs,
    ...
    for entity_annotation_name in span_annotation_names:
        for span_ann in doc.span_annotations(entity_annotation_name) or []:
            serialized_annotations[span_ann] = serialize_labeled_span(
                span_ann, doc, **kwargs
            )
...
) -> List[Tuple[Optional[str], str, List[str]]]:
